...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/07/28 12:14:46 by khirsig           #+#    #+#             */
/*   Updated: 2022/08/08 14:56:22 by khirsig          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main() {
    ...
    if (test.empty()) std::cout << "Vector is empty with size " << test.size() << "\n";
    std::cout << "Size: " << test.size() << "  Capacity: " << test.capacity() << "\n";
    test.reserve(5);
    std::cout << "Size: " << test.size() << "  Capacity: " << test.capacity() << "\n";
    for (std::size_t i = 0; i < test.capacity(); ++i) test.push_back(i + 1);
    test.resize(10, 42);
    std::cout << "Size: " << test.size() << "  Capacity: " << test.capacity() << "\n";
    std::cout << "Print all ints:"
              << "\n";
    for (std::size_t i = 0; i < test.size(); ++i) std::cout << test[i] << " ";
    std::cout << "\n";
    test.pop_back();
    test.pop_back();
    std::cout << "Size: " << test.size() << "  Capacity: " << test.capacity() << "\n";
    if (!test.empty()) std::cout << "Vector is not empty with size " << test.size() << "\n";
    ...
    for (VEC::iterator i = cpy.begin(); i != cpy.end(); ++i) std::cout << *i << " ";
    std::cout << "\n";
    cpy.insert(cpy.begin() + 2, 23);
    for (std::size_t i = 0; i < cpy.size(); ++i) std::cout << cpy[i] << " ";
    std::cout << "\n";
    std::cout << "Size: " << cpy.size() << "  Capacity: " << cpy.capacity() << "\n";
    ...
    if (NS::is_integral<char>::value) std::cout << "Char is integral\n";
    if (!NS::is_integral<float>::value) std::cout << "Float is not integral\n";
    if (NS::equal(test.begin(), test.end(), cpy.begin())) std::cout << "Cpy and Test are equal.\n";
